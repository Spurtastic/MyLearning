---
date: 2024-06-17T10:28
tags:
  - Daily
cssclasses:
  - daily
  - monday
---
# DAILY NOTE
## Monday, June 17th, 2024
***
### Journal
#### Programming fundamentals

I need to find some way to brush up my algorithms and logic systems. 


***
### Tasks
- [ ] Job Applications 50/100
- [ ] Setting up some algorithms folder in here and maybe starting with writing insertion and selection sort. 
- [x] Also some paper to check out logic systems, (A lot of the tests use it)
- [x] Some of the uxcell lessons for the certification
- [x] Clean up the repositories
